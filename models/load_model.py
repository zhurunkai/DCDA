...
from models.cda import CDAClip
...
def load_clip(
    ...
    if not jit:
        model = build_model(
            config, dataset, state_dict or model.state_dict(), context_length
        ).to(device)
        if str(device) == "cpu":
            model.float()
        return model, _transform(model.visual_encoder.model.input_resolution)
...
def build_model(config, dataset, state_dict: dict, context_length=77):
    ...
    for key, value in state_dict.items():
        if key[:6] == "visual":
            new_state_dict.update({"visual_encoder.model" + key[6:]: value})
        elif key == "logit_scale":
            new_state_dict.update({key: value})
        else:
            new_state_dict.update({"text_encoder." + key: value})
