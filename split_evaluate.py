...
def compute_representations(model: DCDA, test_dataset, config):
    split_cal_size = config.split_cal_size

    def batch_cal_after_graph(config, model, x, eot_idx):
        v = torch.Tensor()
        if config.clip_model == 'ViT-L/14':
            final_len = 24
        else:
            final_len = 12
        for j in range(final_len - config.l_adapter_layers, final_len):
            abs = torch.Tensor().to(config.device)
            for i in range(eot_idx.size(0) // split_cal_size + 1):
                step = model.text_encoder.forward_complete_attn((x[:, i * split_cal_size:i * split_cal_size + (
                    split_cal_size if i != eot_idx.size(0) // split_cal_size else eot_idx.size(0) % split_cal_size),
                                                                 :].to(config.device), eot_idx), j)
                abs = torch.cat([abs.to(config.device), step[0]], dim=1)
            v, _ = model.text_encoder.forward_complete_graph((abs, eot_idx), j)
            del abs
            abs = torch.Tensor().to(config.device)
            for i in range(eot_idx.size(0) // split_cal_size + 1):
                step = model.text_encoder.forward_complete_mlp((v[:, i * split_cal_size:i * split_cal_size + (
                    split_cal_size if i != eot_idx.size(0) // split_cal_size else eot_idx.size(0) % split_cal_size),
                                                                :].to(config.device), eot_idx), j)
                abs = torch.cat([abs.to(config.device), step[0]], dim=1)
            del v
            v, _ = model.text_encoder.forward_complete_graph((abs, eot_idx), j)
        v = model.text_encoder.forward_complete_post((v.to(config.device), eot_idx))
        return (v, eot_idx)
    ...
    eot_idx = tokenize_res.argmax(dim=-1)
    abs = torch.Tensor().to(config.device)
    for i in range(tokenize_res.size(0) // 1000 + 1):
        step = model.text_encoder.forward_complete1(tokenize_res[i * 1000:i * 1000 + (
            1000 if i != tokenize_res.size(0) // 1000 else tokenize_res.size(0) % 1000)])
        step = model.text_encoder.forward_complete_block(step)
        abs = torch.cat([abs.to(config.device), step[0]], dim=1)
    rep = batch_cal_after_graph(config, model, abs, eot_idx)[0]
